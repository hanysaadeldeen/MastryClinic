<template>
  <section class="Privacyhero relative startSection">
    <StarterSection :title="$t('privacy.startSection.startSectionTitle')"
      :paragraph="$t('privacy.startSection.startSectionDescription')"
      :buttonTitle="$t('privacy.startSection.startSectionBtn')" headerTitle="mastery">
      <template #background>
        <div class="Starterbackground  relative  w-[600px] h-[600px]">
          <img src="~/assets/img/privacyStarterSection.svg" cla alt="privacyStarterSection"
    ...
    <div class="container relative mx-auto text-white max-w-[1200px] mt-24">
      <div class="relative  px-11 sm:px-7">
        <div class="privacy-page flex" :key="locale">
          ...
          <nav class="hidden lg:block table-of-contents sticky top-0 bottom-0 max-w-[286px]"
            ...
            <ul>
              <li v-for="(section, index) in sections" :key="index">
                <span class="inline-block text-white font-light text-xl mb-5 cursor-pointer"
                  :class="locale === 'en' ? 'text-left' : 'text-right'" @click="scrollToSection(section.id)">
                  {{ $t(section.title) }}
                </span>
              </li>
            </ul>
