    ...
    >
      <div class="relative z-20">
        <div class="flex items-center justify-between">
          ...
          <i
            @click="toggleSidebar"
            class="fa-solid fa-bars-staggered block cursor-pointer text-3xl lg:hidden"
            :class="{ hidden: isSideBar }"
            aria-label="Open menu"
          ></i>
          <i
            @click="toggleSidebar"
            class="fa-solid fa-xmark block cursor-pointer text-3xl lg:hidden"
            :class="{ hidden: !isSideBar }"
...
.gradient-button {
  ...
  background: linear-gradient(#0a0911, #0a0911) padding-box,
    linear-gradient(45deg, rgba(175, 153, 255, 0.5), rgba(255, 244, 34, 0.5))
      border-box;
